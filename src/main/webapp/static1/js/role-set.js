$(function(){
	...
	$(".three").on("click",function(){
		var checkedlength = [];
		var three = $(this).parents('.three-ul').find(".three");
		for(k in three){
			if(three[k].checked){
				checkedlength.push(three[k].value);
			}
		};
		if(checkedlength.length > 0){
			$(".first-div input").prop('checked',true);
			three.parents('.second-ul').siblings('div').find('input').prop('checked',true);
			three.parents('.three-ul').siblings('div').find('input').prop('checked',true);
		}else{
			three.parents('.second-ul').siblings('div').find('input').prop('checked',false);
			three.parents('.three-ul').siblings('div').find('input').prop('checked',false);
		}
	})
